...
const ListItem = ({ data, timeElapsed }) => {
...
const ListOf = ({ data, loading, error }) => {
      ...
      <Widget title={title}>
        <div className="searchbar__container pt-5 pb-10 ">
          {supporters.map(supporterData => {
            return <ListItem data={supporterData} timeElapsed={timeElapsed} />;
          })}
        </div>
      </Widget>
    );
...
const ListOfContainer = () => {
  ...
  return <ListOf data={data} loading={loading} error={error} />;
};

export { ListOf, ListOfContainer };
